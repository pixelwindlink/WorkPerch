# dashboard-legacy-migration Specification
...
Confirmed localStorage migration SHALL construct a `dashboard-key-value-list` version 1 backup and invoke `dashboard.backup.import`, first as dryRun and then as an explicitly confirmed merge commit.
...
Dashboard SHALL accept the existing `format: dashboard-key-value-list`, `version: 1` structure and SHALL fully validate its paths and notes before dry-run or commit.
...
Opening static HTML without Dashboard Engine Server MAY display the UI shell and legacy migration information but MUST NOT allow localStorage-backed business writes or claim migration success.
