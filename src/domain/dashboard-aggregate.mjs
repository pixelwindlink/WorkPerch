...
function enumValue(value, name, values, code) {
  if (!values.has(value)) throw dashboardError(code, `${name} 不受支持。`);
  return value;
}
...
function assertTagReferences(state, item, name, code) {
  ...
  for (const id of item.tagIds) if (!ids.has(id)) throw dashboardError(code, `${name} 引用了不存在的标签 ${id}。`);
}
...
export function assertAggregate(value) {
  ...
  assertRequiredKeys(value, STATE_KEYS, "dashboard state", code);
  if (value.schemaVersion !== "2.0") throw dashboardError(code, `不支持状态 Schema ${value.schemaVersion}。`);
  ...
  ensureArray(value.tags, "tags", LIMITS.tags, code).forEach((item) => {
    ...
    if (!deepSame(item, normalized)) throw dashboardError(code, `标签 ${normalized.id} 不是规范化状态。`);
  });
  ...
  ensureArray(value.savedViews, "savedViews", LIMITS.savedViews, code).forEach((item) => {
    ...
    if (!deepSame(item, normalized)) throw dashboardError(code, `保存视图 ${normalized.id} 不是规范化状态。`);
...
export function migrateAggregateToV2(source, { now, idFactory }) {
  ...
  const code = "DASHBOARD_STATE_CORRUPT";
  assertAllowedKeys(source, ["schemaVersion", "aggregateRevision", "groups", "paths", "notes", "projects", "createdAt", "updatedAt"], "dashboard state 1.x", code);
  if (source.schemaVersion !== "1.0") throw dashboardError(code, `不支持状态 Schema ${source?.schemaVersion}。`);
  assertRequiredKeys(source, ["schemaVersion", "aggregateRevision", "paths", "notes", "projects", "createdAt", "updatedAt"], "dashboard state 1.x", code);
...
export function createInitialAggregate({ now, projects = [], idFactory }) {
  ...
  const normalizedProjects = projects.map((item) => {
    const tagIds = (item.tags || []).map((name) => findOrCreateTag(tags, { name }, { now, idFactory, code: "DASHBOARD_STATE_CORRUPT" }).id);
    return normalizeProjectItem({ ...item, tagIds }, { now, idFactory, legacy: true, code: "DASHBOARD_STATE_CORRUPT" });
  });
...
function resolveTagSelection(state, item, existing, { now, idFactory }) {
  ...
  if (item.tagIds !== undefined) {
    ...
    for (const id of tagIds) if (!tags.some((tag) => tag.id === id)) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `标签 ${id} 不存在。`);
    return { tags, tagIds };
  }
...
export function upsertTag(state, { item, expectedRevision }, { now, idFactory }) {
  ...
  if (item.id && !existing) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `标签 ${item.id} 不存在。`);
  ...
  if (duplicate) throw dashboardError("DASHBOARD_TAG_ALREADY_EXISTS", `标签名称已经由 ${duplicate.id} 使用。`);
...
export function deleteTag(state, { id, expectedRevision }, { now }) {
  assertExpectedRevision(state, expectedRevision);
  if (!state.tags.some((item) => item.id === id)) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `标签 ${id} 不存在。`);
  ...
  if (inUse) throw dashboardError("DASHBOARD_TAG_IN_USE", `标签 ${id} 仍被记录或保存视图引用。`);
...
export function repairPath(state, { id, path: nextPath, expectedRevision, inspection = null }, { now }) {
  ...
  if (!existing) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `路径 ${id} 不存在。`);
...
export function setEntryInspection(state, { kind, id, inspection, expectedRevision }, { now }) {
  assertExpectedRevision(state, expectedRevision);
  if (!["path", "project"].includes(kind)) throw dashboardError("INVALID_PAYLOAD", "inspection.kind 只允许 path 或 project。");
  const collectionName = kind === "path" ? "paths" : "projects";
  const existing = state[collectionName].find((item) => item.id === id);
  if (!existing) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `${kind} ${id} 不存在。`);
...
export function refreshAllPathInspections(state, { expectedRevision, inspections }, { now }) {
  assertExpectedRevision(state, expectedRevision);
  if (!Array.isArray(inspections)) throw dashboardError("INVALID_PAYLOAD", "inspections 必须是数组。");
  const byId = new Map();
  for (const entry of inspections) {
    if (!entry || typeof entry !== "object" || typeof entry.id !== "string") {
      throw dashboardError("INVALID_PAYLOAD", "inspections[].id 不合法。");
    }
  ...
  if (byId.size !== state.paths.length || state.paths.some((item) => !byId.has(item.id))) {
    throw dashboardError("INVALID_PAYLOAD", "全量刷新必须覆盖当前全部路径条目。");
  }
...
export function batchUpsertEntries(state, { items, expectedRevision }, { now, idFactory }) {
  assertExpectedRevision(state, expectedRevision);
  if (!Array.isArray(items) || items.length < 1 || items.length > 50) throw dashboardError("INVALID_PAYLOAD", "batch.items 必须是 1 到 50 项的数组。");
  let candidate = clone(state);
  const accepted = [];
  for (const source of items) {
    if (!source || !["path", "project"].includes(source.target)) throw dashboardError("INVALID_PAYLOAD", "batch item.target 只允许 path 或 project。");
    ...
    if (existingOther) throw dashboardError("DASHBOARD_PATH_ALREADY_EXISTS", `路径已经由 ${existingOther.name} 收录。`);
...
export function upsertProject(state, { item, expectedRevision }, { now, idFactory }) {
  ...
  if (duplicate) throw dashboardError("DASHBOARD_PROJECT_ALREADY_EXISTS", `项目路径已经由 ${duplicate.name} 收录。`);
...
export function recordEntryUsage(state, { kind, id }, { now }) {
  ...
  if (!collectionName) throw dashboardError("INVALID_PAYLOAD", "usage.kind 只允许 path、note 或 project。");
  const existing = state[collectionName].find((item) => item.id === id);
  if (!existing) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `${kind} ${id} 不存在。`);
...
export function upsertSavedView(state, { item, expectedRevision }, { now, idFactory }) {
  ...
  if (item.id && !existing) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `保存视图 ${item.id} 不存在。`);
  const normalized = normalizeSavedView(item, { now, idFactory, existing });
  for (const id of normalized.tagIds) if (!state.tags.some((tag) => tag.id === id)) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `标签 ${id} 不存在。`);
  ...
  if (duplicate) throw dashboardError("DASHBOARD_VIEW_ALREADY_EXISTS", `保存视图名称已经由 ${duplicate.id} 使用。`);
...
export function deleteSavedView(state, { id, expectedRevision }, { now }) {
  assertExpectedRevision(state, expectedRevision);
  if (!state.savedViews.some((item) => item.id === id)) throw dashboardError("DASHBOARD_ITEM_NOT_FOUND", `保存视图 ${id} 不存在。`);
...
export function snapshotOf(state, include = ["tags", "paths", "notes", "projects", "savedViews"]) {
  ...
  include.forEach((name) => {
    if (!STATE_KEYS.includes(name) || ["schemaVersion", "aggregateRevision", "createdAt", "updatedAt"].includes(name)) {
      throw dashboardError("INVALID_PAYLOAD", `snapshot include 不支持 ${name}。`);
    }
