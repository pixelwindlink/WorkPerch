# dashboard-engine-actions Specification
...
Dashboard SHALL declare `engine.describe`, `system.health`, `dashboard.snapshot.get`, path/note/project upsert and delete, `dashboard.project.probe`, `dashboard.backup.export` and `dashboard.backup.import` in one Action Catalog with request and success payload Schema references.
...
Every Action request and success payload SHALL be validated against its JSON Schema 2020-12 file, each Dashboard payload Schema SHALL reject additional properties, and unsupported payloads SHALL not reach a use case.
...
`dashboard.snapshot.get` SHALL return schemaVersion, aggregateRevision, timestamps and the explicitly included public paths, notes and projects without exposing internal files or lock metadata.
...
- **THEN** Dashboard SHALL return `DASHBOARD_PATH_ALREADY_EXISTS` without changing revision
...
- **THEN** Dashboard SHALL return `DASHBOARD_ITEM_NOT_FOUND` without changing revision

### Requirement: Project Actions store launch metadata but never execute it
`dashboard.project.upsert` and `dashboard.project.delete` SHALL manage project name, classification label, description, path, optional loopback URL/port, command string and tags using expectedRevision, while treating commands as inert data.

#### Scenario: Project upsert includes a shell command string
- **WHEN** the Action succeeds
- **THEN** Dashboard SHALL persist and return the string without spawning a process or interpreting shell syntax

### Requirement: Project probe is restricted to registered loopback endpoints
`dashboard.project.probe` SHALL accept only registered project IDs, SHALL probe only their stored HTTP(S) loopback endpoints, SHALL enforce timeout and concurrency limits, and SHALL describe endpoint liveness rather than target Engine health.

#### Scenario: Registered project points to a remote host
- **WHEN** probe policy evaluates the endpoint
- **THEN** Dashboard SHALL return `DASHBOARD_PROBE_FORBIDDEN` and SHALL NOT issue a network request
...
`dashboard.backup.export` SHALL return a complete `dashboard-engine-backup` version 1 payload containing paths, notes and projects from one aggregate revision.
...
`dashboard.backup.import` SHALL accept explicit `merge` or `replace`, explicit dryRun, optional expectedRevision for commit, legacy backup version 1 and Engine backup version 1, and SHALL validate the complete candidate before one atomic state change.

#### Scenario: One imported item is invalid
- **WHEN** any item fails validation
- **THEN** Dashboard SHALL return `DASHBOARD_IMPORT_INVALID` or `DASHBOARD_IMPORT_REJECTED` and SHALL persist none of the import

#### Scenario: Dry run succeeds
- **WHEN** dryRun is true
- **THEN** Dashboard SHALL return planned add/update/skip counts without changing aggregateRevision
...
- **THEN** Dashboard SHALL reject it as a protocol error and SHALL NOT dispatch the Action
