...
description: Safely inspect, check, test, start, call, and diagnose Dashboard Engine while preserving its single-writer state ownership. Use when operating the Dashboard Server, CLI, Web UI, EngineMessage endpoint, temporary runtime, project launcher catalog, or root black-box conformance without opening the same state as competing writers.
---

# Operate Dashboard Engine

Preserve Dashboard's EngineMessage boundary and one state owner.

## Workflow

1. Locate the Dashboard Root by finding `engine.manifest.json` with `id=dashboard`; do not rely on Skill directory depth.
...
5. For Server operation, require an explicit absolute `DASHBOARD_RUNTIME_DIR` unless the user intentionally uses the documented local default.
...
- Never delete `.dashboard-owner.lock` to bypass `STATE_OWNERSHIP_CONFLICT`.
