...
test("manifest 1.2 consumers match Perch-owned facts", async () => {
  const manifest = await readJson(path.join(PERCH_ROOT, "engine.manifest.json"));
  const packageJson = await readJson(path.join(PERCH_ROOT, "package.json"));
  const catalog = await readJson(path.join(PERCH_ROOT, manifest.actions.catalog));
  ...
  for (const reference of Object.values(manifest.consumers.human)) {
    const resolved = path.resolve(PERCH_ROOT, reference);
    const relative = path.relative(PERCH_ROOT, resolved);
    assert.ok(relative.length > 0 && !relative.startsWith("..") && !path.isAbsolute(relative));
    await fs.access(resolved);
  }

  const canonicalSkills = [];
  for (const entry of await fs.readdir(path.join(PERCH_ROOT, ".agents/skills"), { withFileTypes: true })) {
  ...
  for (const skill of manifest.consumers.agent.skills) {
    await fs.access(path.join(PERCH_ROOT, skill.path));
    for (const compatibilityPath of skill.compatibilityPaths ?? []) await fs.access(path.join(PERCH_ROOT, compatibilityPath));
  }
});

test("in-process and CLI describe expose the same schema-safe consumers", async () => {
  const runtimeDir = await tempRuntime("perch-consumers-");
