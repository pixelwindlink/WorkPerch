...
test("CLI stdin emits exactly one success response with isolated stderr", async () => {
  const runtimeDir = await tempRuntime("dashboard-cli-stdin-");
  try {
    ...
    assert.equal(response.payload.id, "dashboard");
  } finally {
...
test("CLI message file supports system.health and returns exit zero", async () => {
  const runtimeDir = await tempRuntime("dashboard-cli-file-");
  ...
  try {
    const result = await runCli({ runtimeDir, args: ["--message-file", messagePath], environment: { DASHBOARD_SERVER_URL: "" } });
    assert.equal(result.code, 0);
    assert.equal(result.stderr, "");
...
test("CLI protocol/business errors use nonzero exit and structured stdout", async () => {
  const runtimeDir = await tempRuntime("dashboard-cli-error-");
  try {
    const unsupported = request("dashboard.unsupported", {}, { id: "cli-error" });
    const result = await runCli({ runtimeDir, input: JSON.stringify(unsupported), environment: { DASHBOARD_SERVER_URL: "" } });
    ...
    const invalidJson = await runCli({ runtimeDir, input: "{invalid", environment: { DASHBOARD_SERVER_URL: "" } });
...
test("CLI reports ownership conflict and never becomes a second writer", async () => {
  const runtimeDir = await tempRuntime("dashboard-cli-lock-");
  const owner = await createDashboardEngine({ mode: "standalone", runtimeDir, genericEnginesRoot: GENERIC_ENGINES_ROOT });
  await owner.start();
  try {
    const message = request("dashboard.snapshot.get", {}, { id: "cli-lock" });
    const result = await runCli({ runtimeDir, input: JSON.stringify(message), environment: { DASHBOARD_SERVER_URL: "" } });
    assert.notEqual(result.code, 0);
    const response = oneResponse(result);
    assert.equal(response.id, message.id);
...
test("configured unavailable Server returns Transport error without local fallback", async () => {
  const runtimeDir = await tempRuntime("dashboard-cli-client-");
  try {
    const message = request("dashboard.snapshot.get", {}, { id: "cli-client" });
    const result = await runCli({
      runtimeDir,
      input: JSON.stringify(message),
      environment: { DASHBOARD_SERVER_URL: "http://127.0.0.1:1" },
    });
    assert.notEqual(result.code, 0);
    assert.equal(oneResponse(result).error.code, "TRANSPORT_ERROR");
    await assert.rejects(() => fs.access(path.join(runtimeDir, "dashboard-state.json")), (error) => error.code === "ENOENT");
  } finally {
    await removeRuntime(runtimeDir);
  }
});
