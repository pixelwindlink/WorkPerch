...
3. <Dashboard工程根目录>、<目标项目根目录> 或 <任务目标> 缺失时，在任何 Dashboard 写入前一次性询问。
...
1. Dashboard manifest 和 `dashboard.snapshot.get` / `dashboard.project.upsert` 请求、响应 Schema。
2. <目标项目根目录> 的 manifest/package、README、AGENT 和真实启动入口。
3. Dashboard 当前 snapshot、aggregateRevision 与已登记 ID/path。
4. <DashboardServerURL> 或明确无 Server owner 的 <DashboardRuntime根目录>。

## 任务目标

为 <目标项目根目录> 完成 <任务目标>，优先通过 <DashboardServerURL> 调用 Dashboard；只有显式提供且确认无 Server owner 的 <DashboardRuntime根目录> 才允许 standalone，并按 <置顶偏好> 处理 pinned。

## Input Contract

- Dashboard 与目标项目根：<Dashboard工程根目录>、<目标项目根目录>。
- 目标：<任务目标>。
...
1. 校验 Dashboard ID 和 Action Schema，采集目标项目稳定 ID、名称、真实分类、路径、URL/port/command/tags。
2. 优先 Server Client；Server 不可用且没有权威 Runtime Root 时停止。
3. 调用 `dashboard.snapshot.get`，按 ID 和规范化绝对路径决定 add/update，阻断身份冲突。
4. 使用 aggregateRevision 调用 `dashboard.project.upsert`；revision conflict 只允许刷新后重试一次。
...
- 不编辑 Dashboard seed/source/state/lock/backup/Registry/Schema，不执行项目 command。
