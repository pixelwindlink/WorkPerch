...
> 非运行声明：本文件仅供人类阅读，不参与 Perch Skill 触发、Server/CLI 启动、EngineMessage 调用或状态 ownership 判定。

## 基本信息

| 项目 | 内容 |
|---|---|
| Skill ID | `operate-perch` |
| 所属项目 | `perch` Engine |
...
用于安全检查、测试、启动、调用和诊断 WorkPerch，覆盖 Server、CLI、Web UI、EngineMessage endpoint、临时 runtime、项目入口目录和根黑盒 conformance。

## 负责什么

- 通过 manifest `id=perch` 定位 Engine Root。
...
- Perch Root、manifest、AGENT、README 和 operations Contract。
- 当前 Git/OpenSpec 状态。
- Server 模式的 URL，或明确、绝对、隔离的 `PERCH_RUNTIME_DIR`。
...
- 根 conformance 是否真正执行或跳过 Perch。

## 安全与状态边界

- Perch 是 `single-writer` Engine；Server 和 Standalone 不得共享同一状态根。
- CLI stdout 只能有一个 EngineMessage，诊断写 stderr。
- 用户路径、目录状态、锁和备份不得进入 Git。

## 权威与维护规则

运行权威是同目录 `SKILL.md`、Perch `AGENT.md`、manifest 和 operations Contract。本说明书不拥有 Perch Action、状态格式或 Server ownership 语义，也不证明 Perch 已通过黑盒 conformance。
