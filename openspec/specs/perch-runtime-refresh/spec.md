# perch-runtime-refresh Specification

## Purpose
TBD - created by archiving change improve-perch-runtime-refresh. Update Purpose after archive.
## Requirements
### Requirement: Write success prefers payload-driven local refresh
After a successful mutating Perch Action, the Web UI MUST update client state from the Action success payload when that payload is sufficient, and MUST NOT unconditionally re-fetch the full workspace snapshot for every write. The UI MUST fall back to `perch.snapshot.get` when the payload cannot safely reconstruct Tag Registry or multi-collection changes.
...
- **AND** does not call `perch.snapshot.get` for that refresh
...
- **THEN** the UI refreshes through `perch.snapshot.get`
...
- **THEN** the UI presents guidance that launch/stop requires the Perch desktop app with Launcher resources
