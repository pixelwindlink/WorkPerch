...
> 非运行声明：本文件仅解释 Perch 项目入口登记，不参与 Skill trigger、项目事实采集、EngineMessage 构造或 Perch 状态写入。
...
| 所属项目 | `perch` Engine |
| 类型 | Capability / Perch 项目入口登记 |
| canonical 运行入口 | 同目录 `SKILL.md` |

## 用途与适用场景

用于把当前工程新增或更新到 WorkPerch 的项目启动目录。所有写入通过 `perch.snapshot.get` 和 `perch.project.upsert` EngineMessage Action 完成。

## 负责什么

- 解析 Perch Root 并读取 manifest 与相关 Action Schema。
...
- 不编辑 Perch seed、源码、state、lock、backup、Registry 或 Action Schema。
...
- WorkPerch 的 canonical manifest 和 snapshot/upsert Schema。
- 当前工程的稳定 ID、名称、真实分类、描述、绝对路径、URL、port、command、tags。
- 可用 Perch Server URL，或明确无 Server owner 的权威绝对 runtime root。

## 典型执行流程

1. 解析唯一 Perch Root 和公开 Contract。
2. 从当前工程公开事实构造项目资料。
3. 优先连接 Perch Server；不可用时按 ownership 规则停止或获取授权。
4. 调用 `perch.snapshot.get` 获取 projects 和 `aggregateRevision`。
5. 按 ID/path 决定新增或更新，处理 identity conflict。
6. 调用 `perch.project.upsert`；revision conflict 最多刷新后重试一次。
7. 再次读取 snapshot 验证全部字段和 revision。

## 输入

- 当前项目事实与 Perch Transport 配置。
...
运行权威是同目录 `SKILL.md` 和 Perch Action Schemas。legacy `skills/register-project-entry/` 只是 compatibility adapter，不维护第二份说明书或流程。本文件存在不表示项目已经成功登记。
