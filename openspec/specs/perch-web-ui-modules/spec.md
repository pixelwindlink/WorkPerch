# perch-web-ui-modules Specification

## Purpose
TBD - created by archiving change split-perch-web-ui. Update Purpose after archive.
## Requirements
### Requirement: Web UI loads through a modular composition entry
Perch Web UI MUST load through a single ES Module composition entry (`app.js`) that imports focused UI modules. The UI MUST NOT require a bundler or new production dependencies. Business state MUST continue to come only from Perch EngineMessage Actions; modules MUST NOT open a parallel writer or persist aggregate data in `localStorage`.

#### Scenario: Module entry is used by the HTML shell
- **WHEN** the Perch page is served
...
- **WHEN** Perch Server or Desktop Host serves the Web UI
...
- **WHEN** a successful `perch.snapshot.get` response updates client state
...
- **AND** on confirm, the client sends `perch.tag.delete` with `expectedRevision`
...
- **THEN** the UI MUST NOT send `perch.tag.delete`
