...
4. 可选值采用默认并在结果中列出。
5. 绑定不能绕过 Perch 单写入者、AGENT、Action Schema 或安全边界。

## 使用的 Skill

- Skill：`$operate-perch`
- 所属项目 ID：`perch`
- canonical `SKILL.md`：`.agents/skills/operate-perch/SKILL.md`
- 在 <Perch工程根目录> 完整读取 canonical Skill；不可访问时返回 `SKILL_UNAVAILABLE`。

## 必须读取的权威

1. <Perch工程根目录> 的 `AGENT.md`、README、`engine.manifest.json`。
...
1. 校验 manifest `id=perch`，读取所有操作 Contract 和工作树。
...
- 不删除 `.perch-owner.lock`，不把用户状态写入源码/Git，不执行 catalog command。
...
- 本 Prompt 不参与 Perch Skill trigger 或 Runtime ownership。
