# perch-engine-state Specification

## Purpose
TBD - created by archiving change convert-perch-application-to-engine. Update Purpose after archive.
## Requirements
### Requirement: Perch owns one versioned aggregate
Perch SHALL persist schemaVersion, aggregateRevision, paths, notes, projects, createdAt and updatedAt as one Engine-owned aggregate whose schema and domain invariants are validated on every load and commit.

#### Scenario: A new empty runtime root starts
- **WHEN** no state file exists and Perch owns the root
- **THEN** Perch SHALL atomically create schema version 1.0 state with revision zero and configured first-run project seed
...
- **THEN** Perch SHALL accept it without rewriting state, and groupColor SHALL remain optional until the path is edited

### Requirement: Aggregate owns one shared Group Registry
Perch aggregate SHALL persist a groups collection with unique IDs and normalized names, and every registry-aware Path SHALL reference an existing Group through groupId while retaining a synchronized compatibility group name.
...
Perch SHALL use `PERCH_RUNTIME_DIR` as the runtime-root override, SHALL require an absolute isolated directory for Standalone CLI and tests, and SHALL never commit state, locks, backups or user data to Git.

#### Scenario: Conformance starts Perch
...
Perch Server SHALL be the normal single writer; CLI SHALL either call that Server or acquire exclusive ownership of an explicitly isolated Standalone root, and a second writer SHALL receive `STATE_OWNERSHIP_CONFLICT`.
...
Invalid JSON, unsupported schemaVersion or aggregate invariant failure SHALL produce `PERCH_STATE_CORRUPT`; Perch SHALL not replace the state with defaults merely because it cannot load it.
...
- **THEN** the second SHALL receive `PERCH_REVISION_CONFLICT` and SHALL reload before retrying
...
- **THEN** Perch SHALL restore projects from state and SHALL NOT reapply the source seed
...
#### Scenario: Perch is checked out under another home directory
